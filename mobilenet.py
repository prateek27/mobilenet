...
"""
from keras.layers import ZeroPadding2D,Conv2D,BatchNormalisation,Input,Dropout,DepthwiseConv2D
...
def MobileNet(
	...
	#Block 13
	x = depthwise_conv_block(x,1024,width_multiplier,depth_multiplier,strides=(2,2),block_id=13)
	...
	model = models.Model(inputs,outputs=x,name='mobilenet')

	if weights in not None:
...
def conv_block(inputs,filters,width_multiplier,kernel_size=(3,3),strides=(1,1)):
	...
	x = BatchNormalisation(name='conv1_bn')(x)
...
def depthwise_conv_block(inputs,pointwise_conv_filters,width_multiplier,depth_multiplier=1,strides=(1,1),block_id):
	...
	x = BatchNormalisation(name='conv_dw_%d_bn'%block_id)(x)
	...
	x = BatchNormalisation(name='conv_pw_%d_bn'%block_id)(x)
